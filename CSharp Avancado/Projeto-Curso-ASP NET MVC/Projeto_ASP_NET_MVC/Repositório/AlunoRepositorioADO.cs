...
namespace RepositorioADO
{
    public class AlunoRepositorioADO : IRepositorio<Aluno>
    {
        ...
        private void Atualiza(Aluno aluno)
        {
            ...
            strQueryUpdate += string.Format("WHERE AlunoId = '{0}' ", aluno.Id);
        ...
        {
            using (contexto = new Contexto()) {
                var strQueryDelete = string.Format("DELETE FROM aprendendoASP.dbo.aluno WHERE AlunoId = '{0}' ", entidade.Id);
        ...
        public Aluno ListarPorId(string id)
        {
            using (contexto = new Contexto())
            {
                var strQuerySelect = string.Format("SELECT * FROM aprendendoASP.dbo.aluno WHERE AlunoId = {0} ", id);
                var retorno = contexto.ExecutaComandoComRetorno(strQuerySelect);
